...
- Monorepo root version `0.1.3` → `0.1.4` → `0.1.5` (patch: pi-ai `@smithy/node-http-handler` dependency for CI build).
- **Directory changelogs** — `docs/CHANGELOG.md`, `packages/ai/CHANGELOG.md`, `packages/cli/CHANGELOG.md`, `packages/mom/CHANGELOG.md` updated; see sections above (including PR #4 `engines.node` follow-up).
