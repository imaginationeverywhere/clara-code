...
- **`@imaginationeverywhere/clara-voice-client`** — `greet` caches the first successful greeting in XDG or `~/.cache/clara-code` and replays from disk on later runs; depends on the new shared client package in `packages/clara-voice-client`.
