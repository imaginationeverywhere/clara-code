/**
 * POST {base}/voice/converse — cp-team / quikvoice. Body shape may evolve; keep fields optional.
 * @see prompts/.../01-cp-team-directive-voice-conversation-backend.md
 */
...
export type ConverseRequestBody = {
	...
	text?: string;
	session_id?: string;
	/** @deprecated until backend defines metadata slot */
	extra?: Record<string, unknown>;
};

export type ConverseSuccess = {
	ok: true;
	/** Transcript or reply text when the server returns JSON */
	reply_text?: string;
...
export type ConverseFailure = {
	ok: false;
	/** Set when fetch failed (network / DNS) — caller may stay silent in UI */
	offline?: boolean;
	error: string;
};
...
/**
 * Calls quikvoice (or proxy) `POST /voice/converse` with JSON body.
 * On network errors returns `{ ok: false, offline: true }` without throwing.
 */
export async function postVoiceConverse(
	...
	try {
		...
		/** Binary audio: caller may decode from response bytes via a separate request path; here we only handle JSON mode */
		if (contentType.startsWith("audio/")) {
			return {
				ok: true,
				mime_type: contentType,
				reply_text: undefined,
				rawJson: { _note: "audio/ response body: prefer fetch+arrayBuffer in a thin wrapper" },
			};
		}
